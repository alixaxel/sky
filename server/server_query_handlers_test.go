...
// Ensure that we can restrict a query to a given timestamp range.
func TestServerTimestampRangeQueryWithNoEventsInRange(t *testing.T) {
	runTestServer(func(s *Server) {
		setupTestTable("foo")
		setupTestProperty("foo", "num", true, "integer")
		setupTestData(t, "foo", [][]string{
		...
		// Run query.
		query := `{"query":{"statements":"SELECT sum(num) AS total"}}`
		resp, _ := sendTestHttpRequest("POST", "http://localhost:8586/tables/foo/query?prefix=a&start=2012-01-05T00:00:00Z&end=2012-01-06T00:00:00Z", "application/json", query)
		assertResponse(t, resp, 200, `{"total":0}`+"\n", "POST /tables/:name/query failed.")
	})
}
...
// NOTE: Temporal loop support removed until the backend is converted to Julia.
/*
...
// NOTE: Histogram support is suspended until backend is moved to Julia.
/*
